...
namespace Colas.Clases.culebrita
{
    class CulebraBiCola
    {
        ...
        private static bool MoverLaCulebrita(BiCola culebra, Point posiciónObjetivo, int longitudCulebra, Size screenSize)
        {
            var lastPoint = (Point)culebra.finalBicola();

            if (lastPoint.Equals(posiciónObjetivo)) return true;

            if (culebra.comprobarany(posiciónObjetivo)) return false;
            ...
            {
                var removePoint = (Point)culebra.quitarFrente();
                Console.BackgroundColor = ConsoleColor.Black;
        ...
        {
            var punteo = 0;
            var velocidad = 150; //modificar estos valores y ver qué pasa //VELOCIDAD CON LA QUE SE MOVERÁ LA CULEBRA
            var posiciónComida = Point.Empty;
            var tamañoPantalla = new Size(60, 20);
            var culebrita = new BiCola();
            var longitudCulebra = 3; //modificar estos valores y ver qué pasa //TAMAÑO CON EL QUE COMENZARÁ LA CULEBRA
            var posiciónActual = new Point(0, 9); //modificar estos valores y ver qué pasa //SON LAS COORDENADAS DONDE COMENZARÁ LA CULEBRA
            culebrita.insertar(posiciónActual);
            var dirección = Direction.Derecha; //modificar estos valores y ver qué pasa //LA CULEBRA COMENZARÁ A MOVERSE A LA DERECHA
            ...
            while (MoverLaCulebrita(culebrita, posiciónActual, longitudCulebra, tamañoPantalla))
            {
                ...
                if (posiciónActual.Equals(posiciónComida))
                {
                    ...
                    longitudCulebra++; //modificar estos valores y ver qué pasa //SI LOS CAMBIO A -- NO CRECE LA CULEBRA
                    punteo += 10; //modificar estos valores y ver qué pasa  //SUBE PUNTOS AL MARCADOR
                    MuestraPunteo(punteo);
                    velocidad -= 5;
                }

                if (posiciónComida == Point.Empty) //entender qué hace esta linea  //MARCA LA POSICIÓN EN LA QUE APARECERÁ LA COMIDA
                {
